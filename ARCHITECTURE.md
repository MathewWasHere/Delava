...
The visual identity is derived from the restaurant's physical signage:

- The wordmark in `public/brand/logo-word.png` was extracted from the channel-letter
  sign photo (background removed, glow preserved), so the digital brand is literally
  the same mark customers see on the street.
- Hero and about imagery are AI-enhanced versions of the storefront and round hanging
  sign, colour-graded to the same warm amber / black night palette.
- Food photography is generated in one consistent style: black background, amber rim
  light, shallow depth of field.

Palette hierarchy is enforced as **BLACK → WHITE → ORANGE ACCENT**; orange is an accent,
never a flood fill.
