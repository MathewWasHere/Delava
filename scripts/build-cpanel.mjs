...
const log = (m) => console.log(`\x1b[38;5;208m▲ DELAVA\x1b[0m ${m}`);
...
const HTACCESS = `# ---------------------------------------------------------------------------
# DELAVA — فست فود دلاوا | cPanel configuration
# ---------------------------------------------------------------------------
...
const README = `# 📦 آپلود دلاوا روی cPanel — Upload guide
...
2. Go to **public_html** (or \`public_html/delava\` for a subfolder demo).
3. Click **Upload** and select **delava-cpanel.zip**.
...
try {
  ...
  log("zipping…");
  const zipPath = path.join(root, "delava-cpanel.zip");
  ...
  log(`Zip    : delava-cpanel.zip  (${size} MB)`);
