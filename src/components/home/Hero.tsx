 ...
 * HERO
 *
 * One landscape composition (`delawa_combined`) at three resolutions — a phone
 * sees the same storefront framing as desktop, just fewer pixels. Nothing is
 * re-cropped, stretched or zoomed.
 *
 * NO GRADIENTS anywhere. Readability over the photograph comes from:
 *   - placing the CTAs on the darkest region of the image (lower band)
 *   - a flat translucent scrim behind them, not a gradient
 *   - solid-filled buttons with their own contrast
 *
 * The «فست فود دلاوا» headline is gone; the tagline now sits directly beneath
 * the image with intentionally small spacing.
 */
  ...
  return (
    <section className="bg-ink-950 lg:px-6 lg:pt-5">
      {/* ---------- Image + overlaid CTAs ----------
          ...
          cropping the height) keeps the entire storefront composition intact
      ...
      <div className="relative mx-auto w-full lg:max-w-5xl lg:overflow-hidden lg:rounded-2xl">
        <picture>
          ...
          <img
            ...
            alt="نمای شبانه فست فود دلاوا در فسا"
            width={1600}
            height={878}
            fetchPriority="high"
            decoding="async"
        ...
        {/* CTAs sit ON the image (mobile + desktop). Anchored to the lower edge,
            ...
            storefront and sign are never covered. */}
